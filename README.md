...
### Закрытый запуск на сервере

Текущий серверный экземпляр доступен по адресу
`https://profit.46-8-98-79.sslip.io`. Nginx-конфигурация для отдельного HTTPS-хоста
находится в `deploy/nginx.conf`.

Готовый unit-файл находится в `deploy/wb-profit-firewall.service`. Он оставляет
приложение на `127.0.0.1:3847`: не публикуйте этот интерфейс напрямую в интернет,
поскольку браузер передаёт через него финансовые и write-токены WB. Для доступа с
рабочего компьютера используйте SSH-туннель:

```powershell
ssh -L 3847:127.0.0.1:3847 user@server
```

После подключения откройте `http://127.0.0.1:3847`. Для публичного домена сначала
нужны отдельная аутентификация, HTTPS, ограничение запросов и журнал действий.

Read-only проверку уже настроенных на сервере токенов можно выполнить скриптом
`deploy/live-smoke.mjs`. Он не печатает токены и финансовые суммы и не сохраняет
историю; пути и имена переменных передаются через окружение.
...
### Источники каталога и покрытие

Полный каталог репрайсера собирается из двух официальных методов WB:

- Content API возвращает карточки, названия, бренды, артикулы продавца и `nmID`;
- Prices API возвращает цены, скидки и цены для клиента WB.

Данные объединяются только по `nmID`. Карточка без ценовой записи не удаляется: она отображается со статусом «Цена не задана». Ответ `/api/catalog` также содержит `coverage` с количеством карточек, цен и товаров без цены. Это позволяет отличать неполное покрытие WB от нулевой цены.
