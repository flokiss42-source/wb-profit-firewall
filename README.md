# WB Profit Firewall
## Репрайсер и управление ценами

В интерфейсе доступен отдельный блок репрайсера. После финансового аудита можно загрузить цены товаров через токен категории «Цены и скидки» и увидеть:

- бренд, nmID и текущую цену;
- цену продавца со скидкой и размер скидки;
- цену, отображаемую покупателю WB;
- фильтр по брендам и массовое редактирование цен вручную или через CSV/XLS/XLSX.

Изменения сначала формируются как предпросмотр. Применение требует отдельного явного подтверждения, ограничено защитным шагом и не выполняется автоматически. Стратегия «Конкурент» требует внешнего проверенного источника цен: стандартный API продавца WB цены конкурентов не возвращает.
...
![Репрайсер — цены и скидки товаров](./docs/guide/repricer-full.png)

На скриншоте показан отдельный экран управления ценами: после аудита продавец вводит токен категории «Цены и скидки», загружает товары, фильтрует их по бренду и редактирует цену/скидку перед предпросмотром применения.
## Чем проект отличается

WB Profit Firewall — не облачный сервис аналитики и не демонстрационный Power BI-отчёт. Это локальный инструмент для проверяемого финансового аудита собственного кабинета WB:
...
Проект распространяется по лицензии MIT и предназначен для прозрачной управленческой проверки, а не для автоматического управления ценами или рекламой без подтверждения продавца.
...
### 8. Новые операционные блоки`r`n`r`nЭти блоки добавлены после основного walkthrough из семи шагов:
