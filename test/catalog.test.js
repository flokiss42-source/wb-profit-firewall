...
test('Content API catalog follows cursor and keeps card identity', async () => {
  ...
  const cards = await fetchProductCatalog({ token: 'content-token', fetchImpl: async (_url, options) => {
    ...
    if (calls === 2) {
      ...
      return response({ cards: [{ nmID: 101, title: 'Товар 101', brand: 'Samson', vendorCode: 'S-101' }], cursor: { updatedAt: '2026-08-28T11:00:00Z', nmID: 101 } });
    }
    assert.equal(request.settings.cursor.nmID, 101);
...
test('Content API catalog rejects a repeating cursor', async () => {
  await assert.rejects(fetchProductCatalog({ token: 'content-token', fetchImpl: async () => response({ cards: Array.from({ length: 100 }, (_, i) => ({ nmID: i + 1 })), cursor: { updatedAt: '', nmID: 0 } }) }), /повторяющийся cursor/);
});
