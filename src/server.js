...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && req.url === '/api/catalog') { const input = await body(req); let cards = []; let contentWarning = ''; let prices; try { prices = await fetchPrices({ token: input.priceToken, nmIds: [] }); } catch (error) { throw new Error(`Цены и скидки: ${error.message}`); } try { cards = await fetchProductCatalog({ token: input.contentToken }); } catch (error) { contentWarning = `Content: ${error.message}`; } const byId = new Map(prices.map(item => [item.nmId, item])); const cardById = new Map(cards.map(item => [item.nmId, item])); const products = prices.map(price => ({ ...(cardById.get(price.nmId) ?? { nmId: price.nmId, title: price.vendorCode, brand: '', vendorCode: price.vendorCode, priceStatus: 'content-missing' }), ...price })); return json(res, 200, { generatedAt: new Date().toISOString(), products, warning: contentWarning || null, coverage: { cards: cards.length, prices: prices.length, missingCards: products.filter(item => item.priceStatus === 'content-missing').length } }); }
