...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && req.url === '/api/catalog') { const input = await body(req); let cards; let prices; try { cards = await fetchProductCatalog({ token: input.contentToken }); } catch (error) { throw new Error(`Content: ${error.message}`); } try { prices = await fetchPrices({ token: input.priceToken, nmIds: [] }); } catch (error) { throw new Error(`Цены и скидки: ${error.message}`); } const byId = new Map(prices.map(item => [item.nmId, item])); const products = cards.map(card => ({ ...card, ...(byId.get(card.nmId) ?? { nmId: card.nmId, price: 0, discount: 0, discountedPrice: 0, clientPrice: 0, priceStatus: 'missing' }) })); return json(res, 200, { generatedAt: new Date().toISOString(), products, coverage: { cards: cards.length, prices: prices.length, missingPrices: products.filter(item => item.priceStatus === 'missing').length } }); }
