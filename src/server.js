...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && req.url === '/api/catalog') { const input = await body(req); const [contentResult, priceResult] = await Promise.allSettled([fetchProductCatalog({ token: input.contentToken }), fetchPrices({ token: input.priceToken, nmIds: [] })]); if (priceResult.status === 'rejected') throw new Error(`Цены и скидки: ${priceResult.reason.message}`); const cards = contentResult.status === 'fulfilled' ? contentResult.value : []; const prices = priceResult.value; const contentWarning = contentResult.status === 'rejected' ? `Content: ${contentResult.reason.message}` : ''; const cardById = new Map(cards.map(item => [item.nmId, item])); const priceById = new Map(prices.map(item => [item.nmId, item])); const ids = [...new Set([...cards.map(item => item.nmId), ...prices.map(item => item.nmId)])]; const products = ids.map(nmId => ({ ...(priceById.get(nmId) ?? { nmId, price: 0, discount: 0, discountedPrice: 0, clientPrice: 0, priceStatus: 'missing' }), ...(cardById.get(nmId) ?? { nmId, title: priceById.get(nmId)?.vendorCode ?? `nmID ${nmId}`, brand: '', vendorCode: priceById.get(nmId)?.vendorCode ?? '' }) })); return json(res, 200, { generatedAt: new Date().toISOString(), products, warning: contentWarning || null, coverage: { cards: cards.length, prices: prices.length, missingPrices: products.filter(item => item.priceStatus === 'missing').length } }); }
